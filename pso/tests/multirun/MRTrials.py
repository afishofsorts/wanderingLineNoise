...
##############################################################
# TEST TO SEE HOW RUNTIME AND LSF CHANGE WITH NUMBER OF RUNS #
##############################################################

# note: this was preliminary testing on non distorted data and with a fairly low threshold for clean signals of 0.1.
# As such the exact decay/asymptotic behaviour seen here likely doesn't apply to more realistic applications
...
def PSORunTimes(t, data, Nseg: int, lbounds, ubounds, runs: int):
    ...
    for i in range(runs):
        ...
        for n in range(Nseg):
            if i==0 or isBadFit[n]:
                lt, ut, runSeg, segFit, lbounds, ubounds = pbf.PSOSegmenter(t, data, n, Nseg, lbounds, ubounds) # outdated command
...
for i in range(trials):
    ...
    t, cleanSig, distSig, freqs, freqKnots = sg.genWL(f0, band, M, Ts, A)
